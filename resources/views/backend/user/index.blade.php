    ...
    <div class="row mb-4">
        @foreach($companies as $company)
            <div class="col-md-3 mb-3">
                <div class="card h-100 company-card {{ isset($selectedCompanyId) && $selectedCompanyId == $company->id ? 'border-primary' : '' }}" style="cursor:pointer; {{ isset($selectedCompanyId) && $selectedCompanyId == $company->id ? 'box-shadow: 0 0 0 2px #007bff;' : '' }}" onclick="window.location='{{ route('admin.user.index', ['company_id' => $company->id]) }}'">
                    <div class="card-body d-flex flex-column align-items-center text-center">
                        @if($company->logo)
                            <img src="{{ asset('storage/' . $company->logo) }}" alt="{{ $company->name }}" class="rounded-circle mb-2" width="50" height="50">
                        @else
                        ...
                        <span class="badge bg-info mb-1">Users: {{ $company->users()->count() }}</span>
                        <span class="badge {{ $company->status === 'active' ? 'bg-success' : 'bg-danger' }}">{{ ucfirst($company->status) }}</span>
                    </div>
                </div>
            </div>
        @endforeach
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('admin.user.create', ['company_id' => $selectedCompanyId]) }}" class="btn btn-primary">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        @if($user->picture)
                                            <img src="{{ asset('storage/' . $user->picture) }}" alt="{{ $user->name }}" class="rounded-circle" width="40" height="40">
                                        @else
